...
import { checkMediaDevices, getMediaDevicesWithPermissions, testTURNConnection, checkWebRTCSupport } from '@/lib/utils';
...
const CheckTURNPage = () => {
  ...
  const [mediaStatus, setMediaStatus] = useState<MediaDevicesStatus>({ hasCamera: false, hasMicrophone: false });
  ...
  const requestMediaPermissions = async () => {
    try {
      const stream = await getMediaDevicesWithPermissions(mediaStatus);
      if (stream) {
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      <section style={{ marginBottom: '20px' }}>
        ...
        <p>Обнаружены: Камера - {mediaStatus.hasCamera ? '✅' : '❌'}, Микрофон - {mediaStatus.hasMicrophone ? '✅' : '❌'}</p>
        <p>Разрешения: {permissionsGranted ? '✅' : '❌'}</p>
        
        {!permissionsGranted && (mediaStatus.hasCamera || mediaStatus.hasMicrophone) && (
          <button 
        ...
        {mediaDevices.length > 0 ? (
          <ul>
            {mediaDevices.map((device, index) => (
              <li key={index}>
                {device.kind}: {device.label || `Устройство ${index + 1}`} (ID: {device.deviceId.slice(0, 10)}...)
              </li>
            ))}
          </ul>
        ) : (
      ...
      <section style={{ marginTop: '20px' }}>
        ...
        {candidates.length > 0 ? (
          ...
          <p>Кандидаты не найдены. {isTesting ? 'Идет поиск...' : 'Нажмите "Начать проверку"'}</p>
        )}
      </section>
    </div>
  );
